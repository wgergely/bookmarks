...
{
    // Check if the program has been given enough arguments
    if (argc < 4) {
        std::cerr << "Not enough arguments passed. Usage: <source> <destination> <size>" << std::endl;
        return 1;
    }
    ...
    // Save pointers
    char* source_c = argv[1];
    char* destination_c = argv[2];
    // Parse size
    ...
    if (source_spec.get_int_attribute("oiio:Movie", -1) == 1) {
		...
		if (!is_gif && !codec_name.empty()) {
            std::transform(codec_name.begin(), codec_name.end(), codec_name_lower.begin(), [](unsigned char c){ return std::tolower(c); });
    ...
    std::transform(cspace.begin(), cspace.end(), cspace_lower.begin(), [](unsigned char c){ return std::tolower(c); });
