...
Dist::Paths InitializeEnvironment(bool use_grandparent) {
    ...
    for (const auto &dir : {paths.core, paths.shared, paths.internal, paths.bin}) {
        if (!std::filesystem::exists(dir) || !std::filesystem::is_directory(dir)) {
#ifndef NO_CONSOLE
            MessageBoxW(NULL, (L"A required directory was not found:\n" + dir.wstring()).c_str(), L"Error",
                        MB_ICONERROR | MB_OK);
#endif
    ...
    SetEnvironmentVariableW(
        L"PYTHONPATH",
        (paths.core.wstring() + L";" + paths.internal.wstring() + L";" + paths.shared.wstring()).c_str());

    // Set PATH environment variable
    std::wstring _n_path = (paths.root.wstring() + L";" + paths.bin.wstring() + L";" + paths.internal.wstring() + L";" +
                            paths.shared.wstring() + L";" + paths.core.wstring() + L";");
...
int LaunchProcess(int argc, wchar_t *argv[], std::filesystem::path exe_path) {
    ...
    if (!CreateProcessW(NULL, &cmd[0], NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
        MessageBoxW(NULL, (L"CreateProcess failed (" + std::to_wstring(GetLastError()) + L").").c_str(), L"Error",
                    MB_ICONERROR | MB_OK);
